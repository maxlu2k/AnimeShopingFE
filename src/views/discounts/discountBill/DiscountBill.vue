                        ...
                        <v-card-text>
                            <v-container>
                                <v-form @submit.prevent ref="form" class="form_cua_thang">
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field v-model="item.discountName" label="Tên giảm giá"
                    ...
                    <v-table fixed-header hover density="comfortable" class="custom-table" loading
                        ...
                        <tbody>
                            ...
                            <tr v-else v-for="(item, index) in properties?.data" :key="item.id">
                                ...
                                <td class="text-center">{{ item.discountName }}</td>
                                ...
                                <td class="text-center">
                                    {{ item.discountType == 1 ? formatVND(item.discountAmount) :
...
<script setup>
import { ref } from 'vue';
import { getAll, save, update } from '@/services/discount/discountBillService';
...
const action = ref('Tạo mới giảm giá')
...
const nameValidate = [
    (val) => {
        if (val == null) {
            return "Tên khuyến mại từ 1 - 200 kí tự";
        }
...
const editItem = (i) => {
    action.value = 'Cập nhật  giảm giá';
